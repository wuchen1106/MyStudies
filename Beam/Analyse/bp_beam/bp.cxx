...
int main(int argc, char* argv[]){
	...
	for( Long64_t iEvent = 0; iEvent < (nEvents&&nEvents<nEvent?nEvents:nEvent); iEvent++ ){
		...
		std::vector<double> MT1Monitor_z;
		std::vector<double> MT1Monitor_px;
		...
		fMyRootInterface->get_value("MT1Monitor_y",MT1Monitor_y,cm);
		fMyRootInterface->get_value("MT1Monitor_z",MT1Monitor_z,cm);
		fMyRootInterface->get_value("MT1Monitor_px",MT1Monitor_px,GeV);
		fMyRootInterface->get_value("MT1Monitor_py",MT1Monitor_py,GeV);
		fMyRootInterface->get_value("MT1Monitor_pz",MT1Monitor_pz,GeV);
		...
		bool filled = false;
		for ( int i_mon = 0; i_mon < MT1Monitor_nHits; i_mon++ ){
			filled = true;
			if (verbose >= Verbose_ParticleInfo || iEvent%printModule == 0)
				std::cout<<prefix_ParticleInfoStart
						 <<"  Found Particle! i_mon = "<<i_mon
						 <<", px = "<<MT1Monitor_px[i_mon]
						 <<"MeV, py = "<<MT1Monitor_py[i_mon]
						 <<"MeV, pz = "<<MT1Monitor_pz[i_mon]
						 <<"MeV"
						 <<std::endl;
			x=MT1Monitor_x[i_mon];
			y=MT1Monitor_y[i_mon];
			z=MT1Monitor_z[i_mon];
			px=MT1Monitor_px[i_mon];
