		...
		for( Long64_t iEvent = 0; iEvent < nEvent; iEvent++ ){
			...
			for ( int i_par = 0; i_par<McTruth_nTracks; i_par++){
				...
				if (i_pid==11&&i_ptid==1){
					double px = (*McTruth_px)[i_par];
					double py = (*McTruth_py)[i_par];
					double pz = (*McTruth_pz)[i_par];
			...
			for ( int i_hit = 0; i_hit < CdcCell_nHits; i_hit++ ){
				...
				if (i_tid == tid){
					...
					d_nhits++;
					int ilayer = (*CdcCell_layerID)[i_hit];
					if (ilayer>i_layer_max) i_layer_max = ilayer;
				}
			}
			...
			if ( i_layer_max < 3 ) // at least 4 layers
				continue;
			N8++;
			d_tree->Fill();
