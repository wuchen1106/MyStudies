...
//	int nJobs = 1;
	...
	double NperP = 3535584./1e8;
	TString parName = "OT";
	TString DirName = MyData+"/raw/g4sim/Coll.OT.g60cm10mm.005T.test.g4s.QBH";
	int nProcs = 50;
	int nJobs = 1;
...
//	TString option = "Rmin = 11 cm, L = 50 cm";
	...
	TString runName = parName+"on.test";
	TString option = "test";
...
//	chain2->SetBranchAddress("weight",&weight);
	...
	for (int iEvent = 0; iEvent < nEvents; iEvent++ ){
		...
		// stop
		if ( (*McTruth_pid)[0] != PID && PID ) continue;
		nTotal += weight;
		stopped=false;
		passed=false;
		x = (*McTruth_x)[0]*10;
		...
		if (T_nHits>0){
			if (withStopPosition){
				st_pid = (*T_pid)[0];
				if (st_pid==PID || !PID) stopped=true;
				Ox = (*T_Ox)[0]*10;
				Oy = (*T_Oy)[0]*10;
				Oz = (*T_Oz)[0]*10;
				Ot = (*T_Ot)[0];
