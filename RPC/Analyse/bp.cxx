...
int main(int argc, char* argv[]){
	...
	init_args();
	int result;
	while((result=getopt(argc,argv,"hb:v:n:m:r:p:P:"))!=-1){
		switch(result){
			/* INPUTS */
			case 'm':
	...
	for (int i_begin = 0; i_begin < 100; i_begin++){
		...
		for( Long64_t iEvent = 0; iEvent < (nEvents&&nEvents<nEvent?nEvents:nEvent); iEvent++ ){
			...
			if ( (index_temp = fMyRootInterface->get_TH1D_index("pt_total")) != -1 ){
				fMyRootInterface->get_TH1D(index_temp)->Fill(opt/MeV,Mc_w);
			}
			if ( (index_temp = fMyRootInterface->get_TH1D_index("pz_total")) != -1 ){
				fMyRootInterface->get_TH1D(index_temp)->Fill(opz/MeV,Mc_w);
			}
			if ( (index_temp = fMyRootInterface->get_TH1D_index("y_total")) != -1 ){
				fMyRootInterface->get_TH1D(index_temp)->Fill(oy/mm,Mc_w);
			}
			if ( (index_temp = fMyRootInterface->get_TH2D_index("paVSy_total")) != -1 ){
				fMyRootInterface->get_TH2D(index_temp)->Fill(opa/MeV,oy/mm,Mc_w);
			}
			// Got Cdc Region?
			bool got_CDC = false;
			...
			if (got_CDC){
				...
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pa_CDC")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opa/MeV,CDC_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pt_CDC")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opt/MeV,CDC_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pz_CDC")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opz/MeV,CDC_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("y_CDC")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(oy/mm,CDC_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH2D_index("paVSy_CDC")) != -1 ){
					fMyRootInterface->get_TH2D(index_temp)->Fill(opa/MeV,oy/mm,CDC_w);
				}
			}
			if (got_Target){
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pa_Target")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opa/MeV,Target_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pt_Target")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opt/MeV,Target_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pz_Target")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opz/MeV,Target_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("y_Target")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(oy/mm,Target_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH2D_index("paVSy_Target")) != -1 ){
					fMyRootInterface->get_TH2D(index_temp)->Fill(opa/MeV,oy/mm,Target_w);
				}
			}
			if (got_stopped){
			...
			if (got_recoiled){
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pa_recoil")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opa/MeV,BLT_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pt_recoil")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opt/MeV,BLT_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("pz_recoil")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(opz/MeV,BLT_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH1D_index("y_recoil")) != -1 ){
					fMyRootInterface->get_TH1D(index_temp)->Fill(oy/mm,BLT_w);
				}
				if ( (index_temp = fMyRootInterface->get_TH2D_index("paVSy_recoil")) != -1 ){
					fMyRootInterface->get_TH2D(index_temp)->Fill(opa/MeV,oy/mm,BLT_w);
				}
			}
