...
int main(int argc, char* argv[]){
	...
	std::vector<std::string> Monitor_particleName;
	...
	for( Long64_t iEvent = 0; iEvent < (nEvents&&nEvents<nEvent?nEvents:nEvent); iEvent++ ){
		...
		if (m_MonitorPlane=="McTruth"){
			...
			for ( int i_MP = 0; i_MP<Volumes.size(); i_MP++ ){
				...
				fMyRootInterface->get_value(Volume+"_volName",Monitor_volName);
				fMyRootInterface->get_value(Volume+"_particleName",Monitor_particleName);
...
//								continue;
						...
						fMyRootInterface->set_ovalue("particle",Monitor_particleName[i_mon]);
						// for simulation
						...
						fMyRootInterface->set_ovalue("cvolume",Monitor_volName[i_mon]);
						fMyRootInterface->Fill();
					}
				}
			}
