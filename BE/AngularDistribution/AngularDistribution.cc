...
int main(int argc, char** argv){
	...
	std::vector<int> vec_nsteps;
	//vec_nsteps.push_back(1);
	//vec_nsteps.push_back(10);
	vec_nsteps.push_back(100);

	std::vector<double> vec_trackL;
	vec_trackL.push_back(3.6);
	//vec_trackL.push_back(2.4);
	//vec_trackL.push_back(1.2);
	...
	vec_theta.push_back(0.0);
	vec_theta.push_back(0.1);
	vec_theta.push_back(0.2);
	vec_theta.push_back(0.3);
	vec_theta.push_back(0.4);
	vec_theta.push_back(0.5);
	vec_theta.push_back(0.6);
	vec_theta.push_back(0.7);
	vec_theta.push_back(0.8);
	vec_theta.push_back(0.9);
	vec_theta.push_back(1.0);
	vec_theta.push_back(1.1);
	vec_theta.push_back(1.2);
	...
	for ( int i_trackL = 0; i_trackL < vec_trackL.size(); i_trackL++ ){
		for ( int i_nsteps = 0; i_nsteps < vec_nsteps.size(); i_nsteps++ ){
			//###########################INITIALIZING#######################
			//=>Useful variables
			double stepL = vec_trackL[i_trackL]/vec_nsteps[i_nsteps];
			...
			//==>a:
			double Ek1 = Ek0 - deltaE/vec_nsteps[i_nsteps];
			...
			buff.str("");
			buff.clear();
			buff<<" (E_{k}="<<Ek0<<"MeV, L_{track}="<<vec_trackL[i_trackL]<<"cm, n_{stpes}="<<vec_nsteps[i_nsteps]<<")";
			TString suffexT = buff.str();
			buff.str("");
			buff.clear();
			buff<<"_"<<i_trackL<<"_"<<i_nsteps;
			...
			for ( int i = 0; i < num_evt; i++ ){
				...
				for ( int iStep = 0; iStep < vec_nsteps[i_nsteps]; iStep++ ){
					if(G4UniformRand() < q)
					{
						if(G4UniformRand() < p){
				...
				theta = acos(u[0]);
				h1D2->Fill(theta);
				for ( int iThe = 0; iThe < vec_theta.size(); iThe++ ){
					N_total[iThe]++;
					double theta0 = vec_theta[iThe];
					double xp = sin(theta0)+sin(theta)*cos(phi)*cos(theta0);
					double yp = sin(theta)*sin(phi);
					double zp = cos(theta0)-sin(theta)*cos(phi)*sin(theta0);
					...
					double l = R - r;
					double cosphimax = (d*d+l*l-r*r)/2/d/l;
					double prob;
		...
		}//Finish loop for vec_nsteps
	}//Finish loop for vec_trackL
	...
	for (int i = 0; i < vec_theta.size(); i++ ){
		std::cout<<"P for theta0 = "<<vec_theta[i]<<"rad = "<<N_pass[i]<<", "<<(double)N_pass[i]/N_total[i]<<std::endl;
	}
