...
//	TString name = "1500umSi_1p1_3per_flat";
	TString name = "50umAl_1p09_3per_flat";
	TString DESC = "SF=1.09, FWHM=3%";
	TString type = "50umAl,45deg";
...
//		RMax = 1500;
	...
	for ( int iEvent = 0; iEvent < nEvents; iEvent++ ){
		...
		if (edep)
			nHits = edep->size();
		bool foundMuSC = false;
		bool stopped = false;
		for (int iHit = 0; iHit < nHits; iHit++ ){
			double depth;
			if ((*volName)[iHit]!="MuSC") foundMuSC=true;
			if (type == "1500umSi,45deg"){
				...
				if ((*volName)[iHit]!="Target") continue;
				depth = ((-(*x)[iHit]+(*z)[iHit])/sqrt(2)+50e-4/2)*10000;
			}
			...
			hhh1->Fill(ekin);
			if ((*tid)[iHit]==1){
				stopped = (*Opx)[iHit]==0&&(*Opy)[iHit]==0&&(*Opz)[iHit]==0;
				if (stopped){
		...
		if (foundMuSC) MuSC_count++;
		if (stopped) Stopped_count++;
//		else std::cout<<"Op: ("<<(*Opx)[iHit]<<", "<<(*Opy)[iHit]<<", "<<(*Opz)[iHit]<<")"<<std::endl;
	}
	std::cout<<"MuSC_count = "<<MuSC_count<<", Stopped = "<<Stopped_count<<", ratio = "<<(double)Stopped_count/MuSC_count<<std::endl;
	...
	hhh3->Draw("SAME");
	TLegend *legend = new TLegend(0.5,0.75,0.99,0.9);
