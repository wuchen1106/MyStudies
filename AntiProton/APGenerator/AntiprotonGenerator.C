...
int main(int argc, char* argv[]){
	...
	while(getline(fin_card,s_card)){
		...
		if ( segments[0] == "TH1D" ){
			...
			if(iterator<segments.size()) beamP = string2double(segments[iterator++]); else {std::cout<<"Not enough segments in"<<s_card<<"!!!"<<std::endl; return -1;}
			if(iterator<segments.size()) beamTheta = (string2double(segments[iterator++]))*PI/180.; else {std::cout<<"Not enough segments in"<<s_card<<"!!!"<<std::endl; return -1;}
			if(iterator<segments.size()) beamSpread = string2double(segments[iterator++]); else {std::cout<<"Not enough segments in"<<s_card<<"!!!"<<std::endl; return -1;}
		}
		else{
	...
	if (!strcmp(m_workMode,"com")){
		...
		beam.SetVectM(TVector3(-beamP*sin(beamTheta),0.,beamP*cos(beamTheta)), M_p);
		...
		for( Long64_t iEvent = 0; iEvent < nEvents; iEvent++ ){
			...
			if (weight<=0||weight!=weight) continue;
			N1++;
			...
			while( !pos_OK){
				...
				y0=G4UniformRand()-0.5;
				if ( x0*x0 + y0*y0 <= 0.25 ) pos_OK = true;
			}
			x0 *= beamSpread*2;
			y0 *= beamSpread*2;
			if ( (index_temp = get_TH1D("C_z")) != -1 ){
				z0 = vecH1D[index_temp]->GetRandom();
			}

			// Transmit to real coordinate
			double x,y,z;
			x = x0*cos(beamTheta) - z0*sin(beamTheta);
			y = y0;
			z = z0*cos(beamTheta) + x0*sin(beamTheta);

			// output
			std::cout<<x<<" "<<y<<" "<<z             // x,y,z in mm
				     <<" "<<pDaughter1->Px()*1000.   // px in MeV/c
				     <<" "<<pDaughter1->Py()*1000.   // py in MeV/c
				     <<" "<<pDaughter1->Pz()*1000.   // pz in MeV/c
				     <<" "<<0.                       // t in ns
				     <<" "<<-2212                    // pdgID
				     <<" "<<iEvent                   // event No.
				     <<" "<<1                        // trackID
