...
int main(int argc, char** argv){
	...
	for ( int i = 0; i < num_evt; i++ ){
		...
		theta = acos(u[0]);
		h1D2->Fill(theta);
		double xp = sin(ini_theta)+tan(theta)*cos(phi)*cos(ini_theta);
		double yp = tan(theta)*sin(phi);
		double zp = cos(ini_theta)-tan(theta)*cos(phi)*sin(ini_theta);
		double ap = 1/cos(theta);
		double thetap = acos(zp/ap);
