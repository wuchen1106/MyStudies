...
int main(int argc, char* argv[]){
	...
	if (!strcmp(m_workMode,"com")){
		...
		for( Long64_t iEvent = 0; iEvent < nEvents; iEvent++ ){
			...
			if ( (index_temp = get_TH1D("Ano_Mx")) != -1 ){
				vecH1D[index_temp]->Fill(X.M(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_Mx_log")) != -1 ){
				vecH1D[index_temp]->Fill(X.M(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pa")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Rho(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pa_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Rho(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pt")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pt(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pt_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pt(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pz")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pz(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_pz_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pz(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_vz")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->BoostVector().Z(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_vz_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->BoostVector().Z(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_theta")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Theta(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_theta_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Theta(),weight);
			}
			...
			if (verbose >= Verbose_EventInfo || iEvent%printModule == 0) std::cout<<prefix_EventInfoStart<<"  in CMS "
																										 ...
																										 <<"), E4="<<pDaughter4->E()
																										 <<std::endl;
			if ( (index_temp = get_TH1D("Ano_costhetaCMS")) != -1 ){
				vecH1D[index_temp]->Fill(cos(pDaughter1->Theta()),weight);
			}
			if ( (index_temp = get_TH1D("Ano_paCMS")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Rho(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_paCMS_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Rho(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_ptCMS")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pt(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_ptCMS_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->Pt(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_ECMS")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->E(),weight);
			}
			if ( (index_temp = get_TH1D("Ano_ECMS_log")) != -1 ){
				vecH1D[index_temp]->Fill(pDaughter1->E(),weight);
			}
