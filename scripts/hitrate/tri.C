...
int get_cell_layer(int volID,&cell,&layer){
	for (int i = 0; i< 18; i++){
...
//	nProtons*=19./20;
	...
	std::vector<int> *O_cellID;
	std::vector<int> *O_layerID;
	std::vector<int> *O_triType;
	std::vector<int> *O_triID;
	std::vector<int> *O_triPos;

	std::vector<double> *O_edep;
	std::vector<double> *O_t;
	std::vector<double> *O_px;
	std::vector<double> *O_py;
	std::vector<double> *O_pz;
	std::vector<double> *O_x;
	std::vector<double> *O_y;
	std::vector<double> *O_z;
			...
			for ( int iHit = 0; iHit<C_nHits;iHit++){
				...
				if (tptid != pretid){ // new track
					...
					for(int iMc = McTruth_pid->size()-1; iMc>=0; iMc--){
						if (tptid==(*McTruth_tid)[iMc]||(ppid->size()==0&&tptid==(*McTruth_ptid)[iMc])){
